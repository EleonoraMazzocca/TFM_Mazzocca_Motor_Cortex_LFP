...
#   nohup bash cvae/run_cvae_experiment.sh > experiment.log 2>&1 &
#   tail -f experiment.log        # follow progress
#
# Or inside a tmux/screen session:
#   bash cvae/run_cvae_experiment.sh 2>&1 | tee experiment.log

DATA="${DATA:-/path/to/Separated_Data/classes}"
CKPT="${CKPT:-/path/to/transformer/checkpoint.pt}"
BASE="${BASE:-outputs/broadband6}"
...
run_one() {
    ...
    echo "============================================================"
    if python -m cvae.run_embedding_cvae "$@"; then
        echo "  -> OK: ${label}"
    else
        echo "  -> FAILED: ${label}"
        FAILED+=("${label}")
